...
<%@ page import="java.util.List, com.strutslab.dto.CounterDto, com.strutslab.dto.CounterDetailDto, com.strutslab.form.counter.CounterDetailForm, com.strutslab.util.HtmlUtil" %>
<%
...
<input type="hidden" name="orderNo" value="<%= HtmlUtil.escape(orderNo) %>"/>
...
<table class="form-table">
    ...
    <tr>
        <th>指示日</th>
        <td><%= HtmlUtil.escape(orderDate) %></td>
        <th>指示者</th>
        <td><%= HtmlUtil.escape(issuer) %></td>
    </tr>
    <tr>
        <th>期限</th>
        <td><%= HtmlUtil.escape(deadline) %></td>
        <th>優先度</th>
        <td><%= HtmlUtil.escape(priority) %></td>
    </tr>
    <tr>
        <th>ステータス</th>
...
%>
        <tr>
            ...
            <td><%= HtmlUtil.escape(wc) %></td>
            <td><%= HtmlUtil.escape(person) %></td>
            <td><%= HtmlUtil.escape(dl) %></td>
            <td><%= HtmlUtil.escape(pri) %></td>
            <td><app:statusBadge status="<%= detStatus %>"/></td>
            <td>
                <% if (isComplete) { %>
                    <%= HtmlUtil.escape(actH != null ? actH : "") %>
                <% } else { %>
                    <input type="text" name="actualHours[<%= i %>]" value="<%= HtmlUtil.escape(df.getActualHour(i) != null ? df.getActualHour(i) : "") %>" size="5" placeholder="時間"/>
                <% } %>
            </td>
            <td>
                <% if (isComplete) { %>
                    <%= HtmlUtil.escape(partCode) %>
                <% } else { %>
                    <input type="text" name="usedPartCodes[<%= i %>]" value="<%= HtmlUtil.escape(df.getUsedPartCode(i) != null ? df.getUsedPartCode(i) : partCode) %>" size="10"/>
                <% } %>
            </td>
            <td>
                <% if (isComplete) { %>
                    <%= HtmlUtil.escape(usedQty != null ? usedQty : "") %>
                <% } else { %>
                    <input type="text" name="usedQuantities[<%= i %>]" value="<%= HtmlUtil.escape(df.getUsedQuantity(i) != null ? df.getUsedQuantity(i) : "") %>" size="5" placeholder="数量"/>
                <% } %>
            </td>
            <td>
                <% if (isComplete) { %>
                ...
                <% } else { %>
                    <input type="text" name="notes[<%= i %>]" value="<%= HtmlUtil.escape(df.getNote(i) != null ? df.getNote(i) : note) %>" size="10"/>
                <% } %>
            </td>
