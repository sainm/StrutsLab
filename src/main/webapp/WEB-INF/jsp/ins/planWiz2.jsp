...
<table class="form-table">
    <tr>
        <th>選択設備</th>
        <td><%= HtmlUtil.escape(request.getAttribute("selectedEqpName") != null ? request.getAttribute("selectedEqpName") : "") %></td>
    </tr>
    <tr>
...
%>
            <label style="display:block;padding:4px 0;">
                <input type="radio" name="selectedTmplId" value="<%= HtmlUtil.escape(tid) %>"
                    <%= String.valueOf(tid).equals(selTmpl) ? "checked" : "" %>>
                <%= HtmlUtil.escape(tname) %> (<%= HtmlUtil.escape(kind) %>)
            </label>
<%
        }
    }
